...
router.get('/users', authenticate, async (req, res) => {
  try {
    ...
    // Add following status and filter out current user
    if (results.users.length > 0) {
      ...
      if (userIds.length > 0 && req.user) {
        ...
        // Not authenticated, just add default values
        results.users = results.users.map(user => ({
          ...user,
          isFollowing: false,
          isCurrentUser: false
...
router.post('/reindex-users', async (req, res) => {
  try {
    ...
    try {
      await elasticsearch.client.indices.delete({ index: 'users' });
      logger.info('Deleted existing users index');
    } catch (error) {
    ...
    await elasticsearch.client.indices.create({
      index: 'users',
      body: elasticsearch.USERS_MAPPING
    });
    logger.info('Created users index with correct mapping');
